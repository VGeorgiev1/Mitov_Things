...
for i in range(0,1000):
    ...
    if(math.isnan(error) == False and math.isinf(error) == False):
        if(error + tolerance * error < old_error):
            learning_rate1+=learning_rate1*rate_treshhold
        else:
            learning_rate1-=learning_rate1* 0.4
    old_error = error
    ...
    plt.draw()
    plt.pause(0.1)
    delta[0][0] = learning_rate1 / points * 2 * np.sum(predictions - y)
    delta[1][0] = learning_rate1 / points * 2 * np.sum((predictions - y) * x_big[:,1].reshape(points * 2,1))
    delta[2][0] = learning_rate1 / points * 2 * np.sum((predictions - y) * x_big[:,2].reshape(points * 2,1))
